...
<html>
	...
	<section class="oja">
		...
		<form method="POST" action="deletenoticia.php">
					...
					<tbody id='list'>
						<tr class="atr">
							...
							<?php
								...
								$sql1 = "SELECT id_noticia,titulo,portada,fecha,prioridad FROM noticia";
								$texto2=mysqli_query($con,$sql1) or die(mysqli_error($con));
								while($noticia = mysqli_fetch_array($texto2)){
									$dia=date_create($noticia['3']);
									$daa=date_format($dia,'d/m/Y');
									echo "<tr id='categorie5.1-".$noticia[0]."' data-date='".$noticia[3]."' data-prioridad='".$noticia[4]."' data-alpha='".$noticia[1]."' ><td class='portadaatr'><img src='".$noticia['2']."' class='chiquito' /></td><td><p>".$noticia['1']."</p></td><td class='tituloatr'><p>".$daa."</p></td><td><input name='ids[]' type='checkbox' value='".$noticia['0']."' class='checkatr'/></td></tr>";
								}
							?>
					</tbody>
				</table>
