...
<html>
	...
	<section class="oja">
		...
		<?php
			$query="SELECT texto FROM texto WHERE localizacion='docs'";
			$query2="SELECT texto FROM texto WHERE localizacion='legislacion'";
			require 'conection.php';
			$texto=mysqli_query($con,$query) or die(mysqli_error($con));
			$fila=mysqli_fetch_array($texto);
			$texto2=mysqli_query($con,$query2) or die(mysqli_error($con));;
			$fila2=mysqli_fetch_array($texto2);
			
		
		?>
		
		
		<form action="updatetextdoc.php" method="POST">
			<h2>Documentos</h2>
			<textarea id="summernote" name="documentos" class="textfijaedicion" required><?php echo $fila[0]; ?></textarea>
			<hr style='margin-top:3%;width:90%;margin-left:5%;margin-bottom:5%; '>
			<h2>Legislacion</h2>
			<textarea id="summernote2" name="legislacion" class="textfijaedicion" required ><?php echo $fila2[0]; ?></textarea>
