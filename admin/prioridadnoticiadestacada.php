...
<html>
	<head>
		<title>Prioridad Noticia Destacada</title>
	...
	<?php if (login_check($mysqli) == true) : ?>
		...
		<form action="changeprioridadnoti.php?ant=dest" method="POST">
			<div class="priotabla">
			<table id="tabla" class="newtabla">
				...
				<?php
					...
					$sql1 = "SELECT id_noticia,titulo,portada,prioridad,fecha FROM noticia WHERE destacada=1 ORDER BY prioridad DESC";
					$texto2=mysqli_query($con,$sql1) or die(mysqli_error($con));
					while($noticia = mysqli_fetch_array($texto2)){
						$dia=date_create($noticia['4']);
						$daa=date_format($dia,'d/m/Y');
						echo "<tr id='".$noticia['0']."' data-prioridad='".$noticia['3']."'><td id='img'><img src='".$noticia['2']."' class='chiquito1' /></td><td id='tex'><p>".$daa."</p></td><td id='tex'><p>".$noticia['1']."</p></td><td><input id='".$noticia[0]."".$noticia[0]."' type='hidden'  name='amodificar[]'/></td></tr>";
					}
				?>
		...
		<script>
			...
			if (nombrevar == 1){
				$.notify("Se han realizado los cambios correctamente","success");
			}
			else if (nombrevar == 2){
