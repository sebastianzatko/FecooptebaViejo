...
<html>
	...
	<?php if (login_check($mysqli) == true) : ?>
		...
		<form action="deleteseccion.php" method="POST">
			<div class="divtabla1" style="margin-bottom: 10%; margin-top: 7%;">
				<table class="tablaatr" >
					<tbody id='list'>
						...
						<?php
							...
							$sql1 = "SELECT id_texto,localizacion,texto FROM texto";
							$texto2=mysqli_query($con,$sql1) or die(mysqli_error($con));
							while($section = mysqli_fetch_array($texto2)){
								if(!empty($section[2])){
									if($section[1]=="docs"){
										$name="Documento";
										echo "<tr><td class='tituloatr'>".$name."</td><td><input name='ids[]' type='checkbox' value='".$section['0']."' class='checkatr'/></tr></td>";
										
									}elseif($section[1]=="publicacionprincipal"){
										$name="Pop-up pagina principal";
										echo "<tr><td class='tituloatr'>".$name."</td><td><input name='ids[]' type='checkbox' value='".$section['0']."' class='checkatr'/></td></tr>";
									}else{
										echo "<tr><td class='tituloatr'>".ucfirst($section[1])."</td><td><input name='ids[]' type='checkbox' value='".$section['0']."' class='checkatr'/></td></tr>";
									}
								}
