	...
	<section class="oja">
		...
		<div class="cubalibre">
			<table     style='width: 100%;' >
				<tbody id='list'>
					<tr class="atr">
						<th class="atr">Portada</th>
						<th class="atr">Titulo</th>
						<th class="atr">Fecha</th>
						<th class="atr">Modificar</th>
					</tr>
					<?php
						require 'conection.php';
						$sql1 = "SELECT id_noticia,titulo,fecha,portada,prioridad FROM noticia";
						$texto2=mysqli_query($con,$sql1) or die(mysqli_error($con));
						while($noticia = mysqli_fetch_array($texto2)){
							$dia=date_create($noticia['2']);
							$daa=date_format($dia,'d/m/Y');
							echo "<tr  id='categorie5.1-".$noticia['0']."' data-date='".$noticia['3']."' data-prioridad='".$noticia['4']."' data-alpha='".$noticia['1']."' ><td class='portadaatr'><img src='".$noticia[3]."' class='chiquito' /></td><td class='tituloatr'><p>".$noticia['1']."</p></td><td class='fechaatr' ><p>".$daa."</p></td><td class='modificaratr'><a href='modificarnoti.php?noticia=".$noticia['0']."'>Seleccionar</a></td></tr>";
						}
					?>
				</tbody>
			</table>
