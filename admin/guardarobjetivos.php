...
<html>
	...
	<?php if (login_check($mysqli) == true) : ?>
		...
		<?php
			$quey="SELECT texto FROM texto WHERE localizacion='objetivo'";
			require 'conection.php';
			$texto=mysqli_query($con,$quey) or die(mysqli_error($con));
			$fila=mysqli_fetch_array($texto);
		?>
		
		<form method="POST" action="saveobjetivos.php">
		<h2 class="mensaje2">Objetivos</h2>
		<hr style='margin-top:3%;width:90%;margin-left:5%;margin-bottom:5%; '>
			<textarea  id="summernote" name="objetivo" required><?php echo $fila[0]; ?></textarea>
			<div class="botons">
