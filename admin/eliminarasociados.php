...
<html>
	...
	<?php if (login_check($mysqli) == true) : ?>
		...
		<form method="POST" action="deleteasociados.php">
			
				<div class="divtabla2" style="margin-bottom: 10%; margin-top: 7%;background: #f5f5f5;padding: 2%;" id="der">
					<center><h2 class="h2atr">Asociados</h2></center>
					<table style="width:100%;">
						<tr>
							<th class="thatr">Logo</th>
							<th class="thatr">Nombre</th>
							<th class="thatr">Matricula</th>
							<th class="thatr">Eliminar</th>
						</tr>
						<?php
							require 'conection.php';
							$sql = "SELECT id_asociado,nombre,matricula,logo FROM asociado WHERE categoria=1";
							$texto=mysqli_query($con,$sql) or die(mysqli_error($con));
								while($asociado = mysqli_fetch_array($texto)){
									echo "<tr><td class='portadaatr'><img src='".$asociado[3]."' class='chiquito' /></td><td class='tituloatr'>".$asociado['1']."</td><td class='tituloatr'>".$asociado['2']."</td><td><input type='checkbox' name='asociados[]' value='".$asociado['0']."' class='checkatr'></td></tr>";
							}
						?>
					</table>
				</div>
				<div class="divtabla1" style="margin-bottom: 10%; margin-top: 7%;background: #f5f5f5;padding: 2%;" id="izq">
					...
					<table style="width:100%;">
						...
						<tr>
						<?php
							$sql1 = "SELECT id_asociado,nombre,matricula,logo FROM asociado WHERE categoria=2";
							$texto2=mysqli_query($con,$sql1) or die(mysqli_error($con));
							while($coopteba = mysqli_fetch_array($texto2)){
									echo "<tr><td class='portadaatr'><img src='".$coopteba[3]."' class='chiquito' /></td><td class='tituloatr'>".$coopteba['1']."</td><td class='tituloatr'>".$coopteba['2']."</td><td><input type='checkbox' name='asociados[]' value='".$coopteba['0']."' class='checkatr'></td></tr>";
							}
						?>
					</table>
				</div>
