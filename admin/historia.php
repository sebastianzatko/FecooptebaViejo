...
<html>
	...
	<?php if (login_check($mysqli) == true) : ?>
		...
		<?php
			$quey="SELECT texto FROM texto WHERE localizacion='historia'";
			require 'conection.php';
			$texto=mysqli_query($con,$quey) or die(mysqli_error($con));
			$fila=mysqli_fetch_array($texto);
		
		?>
		<form action="guardarhistoria.php" method="POST" enctype="multipart/form-data" >
		<h2 class="mensaje2">Historia</h2>
			<hr style='margin-top:3%;width:90%;margin-left:5%;margin-bottom:5%; '>
			<textarea name="historia" id="summernote" required  ><?php echo $fila[0]; ?></textarea>
			<div class="botons">
