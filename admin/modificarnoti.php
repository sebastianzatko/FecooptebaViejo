	...
	<?php if (login_check($mysqli) == true) : ?>
		...
		<?php
			...
			if(!empty($fila)){
				...
				$obtener3=mysqli_query($con,$sqlasociados) or die(mysqli_error($con));
				$relacionados=[];
				while($rela=mysqli_fetch_array($obtener3)){
					array_push($relacionados,$rela[0]);
				}
				
				echo "<h2 class='mensaje'>Relacionar con...</h2><div class='pectab1'><h2 class='mensaje2'>Vinculos</h2><table class='pectab3'>";
				$obtener1=mysqli_query($con,$consulta) or die(mysqli_error($con));
						while($asociado = mysqli_fetch_array($obtener1)){
							if($relacionados!=NULL){
									if(in_array($asociado['0'], $relacionados)){
										echo "<tr><td><input type='checkbox' value='".$asociado[0]."' name='asociates[]' checked/></td><td><p>".$asociado[1]."</p></td></tr>";
									}
									else{
						...
						echo "</table></div><div class='pectab2'><h2 class='mensaje2'>Coopteba</h2><table class='pectab4'>";
						$obtener2=mysqli_query($con,$consulta2) or die(mysqli_error($con));
						
						
						while($coopteba = mysqli_fetch_array($obtener2)){
							if($relacionados!=NULL){
								if(in_array($coopteba['0'],$relacionados)){
									echo "<tr><td><input type='checkbox' value='".$coopteba[0]."' name='asociates[]' checked/></td><td><p>".$coopteba[1]."</p></td></tr>";
								}else{
						...
						echo "</table></div><div class='botons'><input class='canc' type='button' value='Cancelar' onclick='window.history.back()' name='action'/><input type='submit' class='boton' value='Cargar'></div></form></section>";
			}else{
	  ...
	  <script>
			$('document').ready(function(){
				document.getElementById("list").innerHTML = ['<img class="thumb" src="<?php echo $fila['4']; ?>" />'];
			});
		</script>
